          ...
          {assets.map((a) => {
            ...
            return (
              <StaggerItem key={a.id}>
                <Pressable className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  <button
                  ...
                  {isOpen && (
                    <div className="px-4 pb-4 border-t border-slate-100 pt-3">
                      <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">Job History</div>
                      {history.length === 0 ? (
                        <p className="text-xs text-slate-400">No linked jobs yet.</p>
                      ) : (
                        <div className="space-y-2">
                          {history.map((j) => (
                            <div key={j.id} className="flex items-center justify-between text-xs bg-slate-50 rounded-lg px-3 py-2">
                              <div className="min-w-0">
                                <span className="font-semibold text-robur-black">{j.job_no}</span>
                                <span className="text-slate-400"> · {j.client_name}</span>
                              </div>
                              <span className="text-slate-400 shrink-0">{j.job_date ? format(new Date(j.job_date), "d MMM yyyy") : ""}</span>
                            </div>
                          ))}
                        </div>
                      )}
