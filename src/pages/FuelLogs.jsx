    ...
    <PageTransition className="p-4 md:p-8 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <Pop className="glass-card p-4">
          ...
          <div className="text-2xl font-semibold text-robur-goldDark tabular-mono">${totals.cost.toFixed(2)}</div>
        </Pop>
        <Pop className="glass-card p-4">
          <div className="text-xs text-muted-foreground font-medium">Total Litres</div>
          <div className="text-2xl font-semibold text-foreground tabular-mono">{totals.litres.toFixed(1)} L</div>
        </Pop>
      </div>
        ...
        <Stagger className="space-y-2">
          {logs.map((l) => (
            <StaggerItem key={l.id}>
              <Pressable className="glass-card glass-lift p-4 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="font-semibold text-foreground">{l.vehicle_rego}</div>
                  <div className="text-xs text-muted-foreground">
                    {l.fuel_date ? format(new Date(l.fuel_date), "d MMM yyyy") : ""} · {l.driver_name || "—"}
                    {l.station ? ` · ${l.station}` : ""}
                  </div>
                  {l.odometer ? <div className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5"><Gauge className="w-3.5 h-3.5" /> <span className="tabular-mono">{l.odometer}</span> km</div> : null}
                </div>
                <div className="text-right shrink-0">
                  <div className="font-semibold text-foreground tabular-mono">${(l.cost || 0).toFixed(2)}</div>
                  <div className="text-xs text-muted-foreground tabular-mono">{(l.litres || 0).toFixed(1)} L</div>
